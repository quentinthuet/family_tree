...
package body Arbre_Binaire is
    ...
    procedure Detruire (Arbre         : in out T_AB; 
        ...
        Temp : T_AB; -- Variable temporaire permettant de faire pointer Arbre
                     -- vers null, et de détruire la mémoire précédemment 
                     -- utilisée par Arbre
    ...
    begin
        ...
        if Arbre /= null then
            ...
            -- Si c'est le premier appel (i.e. première itération) de la 
            -- procédure, on vérifie qu'on cherche bien à détruire l'arbre
            -- depuis sa racine
            if Premier_Appel and then Arbre.all.Parent /= null then
                raise Non_Racine_Exception;
            else
                
                -- On détruit récursivement les sous arbres gauche et droit
    ...
    procedure Ajouter (Noeud           : in out T_AB ;
        ...
        Nouveau_Noeud : T_AB; -- Le noeud que l'on crée pour ajouter la nouvelle
                              -- valeur
    ...
    procedure Supprimer_Noeud (Noeud : in out T_AB) is
    begin
        if Noeud /= null then
            Free (Noeud);
        else
    ...
    function Feuille (Noeud     : in T_AB; 
        ...
        Resultat : T_AB; -- La feuille du Noeud dans la Direction 
    ...
    function Racine (Noeud : in T_AB) return T_AB is
        ...
        Resultat : T_AB; -- La racine de l'arbre contenant Noeud
    ...
    begin
        ...
        if Arbre = null then
            ...
            if Profondeur (Arbre.Sous_Arbre_Gauche) < Profondeur (Arbre.Sous_Arbre_Droit) then
                return Profondeur (Arbre.Sous_Arbre_Droit) + 1;
            else
                return Profondeur (Arbre.Sous_Arbre_Gauche) + 1;
            end if;
        end if;
    ...
    procedure Test (Arbre     : in out T_AB; 
        ...
        begin
            ...
            pragma Assert (Profondeur (Arbre) = 3);
